...
- Delivered: Google Chat app fast router at `https://agentcore-fast-router.vercel.app/api/agentcore-chat`. Gemini 2.5 Flash gives instant shallow replies; deeper tasks dispatch to Cursor async. **This is the primary Chat interface — message the AgentCore app in Google Chat, not the scottishramp@gmail.com human DM.**
- Disabled: inbound human-DM Chat polling and pseudo-synchronous runner loop (too slow). Outbound scheduled check-ins still send via Chat API.
...
- **Chat with AgentCore via the Google Chat app** (search "AgentCore" in Google Chat). Do not message `scottishramp@gmail.com` directly — that slow polling path is disabled.
...
- Switched to bot-only Chat: disabled inbound human-DM polling and pseudo-sync loop; AgentCore Google Chat app is the primary interface.
