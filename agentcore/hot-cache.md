...
- Telegram photos: Gemini 3.6 Flash on Vercel is the only model that sees the image. ACK the webhook first; describe is plain prose / minimal thinking. Durable file+facts happen on `agent-runner.yml` (Drive + registry + Cursor), not the nightly knowledge-content ingest. Diagnose with Vercel logs, not stale `getWebhookInfo` 504s. Playbook: `knowledge/playbooks/telegram-fast-router.md`.
...
- Burning Altar dark mode (2026-08-30): system dark mode is greyscale (`#111`) instead of hugo-bearblog teal. Light mode unchanged. Override in `burningaltar/layouts/partials/custom_head.html`.
- Burning Altar tags (2026-08-30): closed set AI / Family / Health / Philosophy / Theology. Home and `/blog/` show all five; unused filters say no posts yet. Wheel also tagged philosophy. Playbook: `knowledge/playbooks/burningaltar-tags.md`.
